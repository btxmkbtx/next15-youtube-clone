...
因为我们使用 drizzle 已经定义了 src\db\schema.ts,而 ZOD 做 form 验证也需要去定义 schema，使用 drizzle-zod 就是为了把 drizzle 的 schema 转嫁到 ZOD 的 schema 上再利用。<br/>
参考代码：<br/>
src\db\schema.ts 的 videos<br/>
...
将视频封面文件托管在这个平台上=>[Github 授权登录](https://uploadthing.com/dashboard/btxmkbtx-personal-team/zeikdtxvx4)

提供了一个 UI 化组件=>UploadDropzone<br/>
代码应用：src\modules\studio\ui\components\thumbnail-upload-modal.tsx<br/>
拦截器中间件:src\app\api\uploadthing\core.ts
...
upstash 提供了一个异步工作流服务
![alt text](99_screenshot/qstash_workflow_config.png)
必要的安装包：bun add @upstash/workflow@0.2.6<br/>
workflow url 可以指定为我们本地开发时用的临时 domain：https://tightly-prompt-halibut.ngrok-free.app<br/>

以这个 trpc 为例解释流程:src\modules\videos\server\procedures.ts の generateThumbnail<br/>
![alt text](99_screenshot/qstash_workflow_demo.png)
