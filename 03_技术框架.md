...
用来测试中间件拦截器页面的页面:src\app\(home)\protected\page.tsx
...
1. 核心組件导出：src\trpc\init.ts<br/>
   通过 F12 查看 createTRPCRouter，createCallerFactory，baseProcedure 三大核心的使用位置<br/>
...
4. HydrateClient<br/>
   在 @trpc/react-query/rsc 中，createHydrationHelpers 提供的 HydrateClient 是一个专门用于 在 RSC (React Server Components) 环境下实现 TRPC 状态 hydration 的客户端组件。它的核心作用是将服务端预获取的 TRPC 查询状态安全地注入到客户端，因为有了 HydrateClient 的注入，src\app\(home)\client.tsx 中的 useSuspenseQuery 才能够拿得到服务端缓存的数据。
