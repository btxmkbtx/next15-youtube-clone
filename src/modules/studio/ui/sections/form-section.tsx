...
import { THUMANAIL_FALLBACK } from "@/modules/videos/constants";
...
const FormSectionSuspense = ({ videoId }: FormSectionProps) => {
  ...
  // 这里其实把categories跟video分开在两个组件里更合理，但是偷懒了没分开
  const [video] = trpc.studio.getOne.useSuspenseQuery({ id: videoId });
  const [categories] = trpc.categories.getMany.useSuspenseQuery();

  const update = trpc.videos.update.useMutation({
    onSuccess: () => {
      // invalidateQueries can invalidate and refetch single or multiple queries in the cache
  ...
  // 如果想要使用form.formState中的各种状态，如isSubmitting，这里需要变成async await模式
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-x-2">
              {/* 如果想要使用form.formState.isSubmitting，onSubmit需要变成async await模式 */}
              <Button
                type="submit"
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="p-0.5 border border-dashed border-neutral-400 relative h-[84px] w-[153px] group">
                        <Image
                          src={video.thumbnailUrl || THUMANAIL_FALLBACK}
